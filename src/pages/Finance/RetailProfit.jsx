...
const RetailProfit = () => {
  ...
  const hasPermission = isAdmin || (user?.permissions || []).includes('finance.profit.retail.view');
    ...
    return (
      <div style={{ padding: '24px' }}>
        <Card>
          ...
          <p>Bạn không được phép truy cập trang Lợi Nhuận Đơn Lẻ. Vui lòng liên hệ quản trị viên để được cấp quyền.</p>
        </Card>
      </div>
    );
  }
  ...
  useEffect(() => {
    const externalRef = ref(database, 'retailExternalCostSettings');
  ...
  const calculateExternalCosts = (order, revenue) => {
    ...
    const storeSettings = externalCostSettings?.[storeKey];
    if (!storeSettings) {
      return { totalExternalCost: 0, breakdown: {} };
    }

    const breakdown = {};
    const addCost = (key, config) => {
      if (!config || config.enabled !== true) return;
      const rawValue = parseFloat(config.value) || 0;
      if (!rawValue) return;
      const amount = (config.type === 'percentage')
        ? (revenue * rawValue) / 100
        : rawValue;
      if (!amount) return;
      breakdown[key] = (breakdown[key] || 0) + amount;
    };

    if (storeSettings.costConfigs) {
      Object.entries(storeSettings.costConfigs).forEach(([key, config]) => addCost(key, config));
    }

    if (storeSettings.customCosts) {
      storeSettings.customCosts.forEach(custom => addCost(custom.label || custom.key, custom));
    }
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Lợi nhuận Bán Lẻ');
    XLSX.writeFile(wb, `loi-nuan-ban-le-${dayjs().format('YYYY-MM-DD')}.xlsx`);
  };
  ...
  return (
    <div style={{ padding: '24px', background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <ExternalCostSettings
        ...
        onSave={() => {}}
        firebasePath="retailExternalCostSettings"
      />
